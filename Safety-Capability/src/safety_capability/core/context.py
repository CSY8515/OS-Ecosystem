"""Common request and execution context models."""
...
class SafetyExecutionContext:
    """Module-neutral data supplied to every safety execution."""
    source: str
    target: str
    action: str
    ...
    def validate(self) -> None:
        """Validate common envelope rules, never module schemas."""
        ...
        if not isinstance(self.timestamp, datetime) or self.timestamp.tzinfo is None:
            raise ContextValidationError("timestamp must be a timezone-aware datetime")
        if self.capability_version != CAPABILITY_VERSION:
            raise ContextValidationError(
                f"capability_version must be {CAPABILITY_VERSION}"
            )

@dataclass(frozen=True, slots=True)
class SafetyRequest:
    """Public request containing context and optional component routing."""
    ...
    def validate(self) -> None:
        ...
        if self.component_id is not None:
            if not isinstance(self.component_id, str) or not self.component_id.strip():
                raise ContextValidationError(
                    "component_id must be a non-empty string when supplied"
                )
