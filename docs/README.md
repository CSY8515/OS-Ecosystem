# OS Ecosystem Documentation

Version: v0.7.1

`docs/` is the single management location for official OS Ecosystem documentation.

## Architecture

- [Architecture](./architecture/ARCHITECTURE.md)
- [Master Design](./architecture/MASTER_DESIGN.md)
- [Common UI System](./architecture/UI_SYSTEM.md)
- [Metadata Contract](./architecture/METADATA_CONTRACT.md)
- [Navigation Contract](./architecture/NAVIGATION_CONTRACT.md)
- [Roadmap](./architecture/ROADMAP.md)
- [Structure](./architecture/STRUCTURE.md)

## Governance

- [Constitution](./governance/CONSTITUTION.md)
- [Rules](./governance/RULES.md)
- [Principles](./governance/PRINCIPLES.md)
- [Standards](./governance/STANDARDS.md)
- [Policies](./governance/POLICIES.md)
- [Decisions](./governance/DECISIONS.md)
- [Conventions](./governance/CONVENTIONS.md)
- [Responsibility Boundary](./governance/RESPONSIBILITY_BOUNDARY.md)

## Registry

- [Project Registry](./registry/PROJECT_REGISTRY.md)
- [Capability Registry](./registry/CAPABILITY_REGISTRY.md)
- [Version Registry](./registry/VERSION_REGISTRY.md)
- [Release Registry](./registry/RELEASE_REGISTRY.md)
- [Contract Registry](./registry/CONTRACT_REGISTRY.md)
- [Route Registry](./registry/ROUTE_REGISTRY.md)

## Release

- [v0.7.1 Release Notes](./release/RELEASE_NOTES_v0.7.1.md)
- [v0.7.1 Release Review](./release/RELEASE_REVIEW_v0.7.1.md)
- [v0.7.0 Release Notes](./release/RELEASE_NOTES_v0.7.0.md)
- [v0.7.0 Release Review](./release/RELEASE_REVIEW_v0.7.0.md)
- [Version History](./release/VERSION_HISTORY.md)
- [v0.6.2 Release Notes](./release/RELEASE_NOTES_v0.6.2.md)
- [v0.6.1 Release Notes](./release/RELEASE_NOTES_v0.6.1.md)
- [v0.6.0 Release Notes](./release/RELEASE_NOTES_v0.6.0.md)
- [v0.5.0 Release Notes](./release/RELEASE_NOTES_v0.5.0.md)
- [v0.4.4 Release Notes](./release/RELEASE_NOTES_v0.4.4.md)

## Capabilities

- [Documentation Standard](./capabilities/README.md)
- [Safety](./capabilities/safety/README.md)
- [Enhancement](./capabilities/enhancement/README.md)
- [Automation](./capabilities/automation/README.md)
- [Collaboration & Connectivity](./capabilities/collaboration-connectivity/README.md)
- [Personal Secretary](./capabilities/personal-secretary/README.md)

## Management principles

1. Official documents use the narrowest `docs/` category.
2. UI, Registry, and Contract use the same classification, version, and Route language.
3. Relative internal links are automatically validated.
4. Capability documents retain the common standard and capability-owned contracts.
5. `PRINCIPLES.md` remains the approved source; the separate official six operating principles follow after source confirmation.
